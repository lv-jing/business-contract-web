  ...
  <div class="bg-white">
    <el-card class="box-card" shadow="never">
      <div slot="header">
        ...
        <span style="font-size: 12px;float: right;">合同流水号：{{ ruleForm.serialNumber }}</span>
      </div>
      <div class="form-bg">
            ...
            <el-col :span="22">
              <el-form-item label="合同描述" prop="contractDesc">
                <el-input disabled type="textarea" maxlength="80" :autosize="{ minRows: 1, maxRows: 4}"
                          ...
                          placeholder="请输入合同描述，字数不超过80字"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            ...
            <el-col :span="10" :offset="2">
              <el-form-item label="合同类型" prop="contractTypeCode">
                <el-select disabled v-model="ruleForm.contractTypeCode"
          ...
          <el-row :gutter="24">
            <el-col :span="10">
              <el-form-item label="我方主体" prop="myEntityCode">
                <el-select disabled multiple v-model="ruleForm.myEntityCode"
            ...
            <el-row :gutter="24">
              ...
              <el-col :span="10" :offset="2">
                <el-form-item label="原币金额" prop="originalCurrency">
                  <el-input disabled
                            v-model="ruleForm.originalCurrency"
                            placeholder="请输入原币金额"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="24">
              <el-col :span="10">
                <el-form-item disabled label="不含税额" prop="excludedAmount">
                  <el-input disabled v-model="ruleForm.excludedAmount" placeholder="请输入不含税额">
              ...
              <el-col :span="1">
                <el-form-item label="" prop="taxRate" label-width="0">%</el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="24">
              <el-col :span="10">
                <el-form-item label="含税金额" prop="includedAmount">
                  <el-input disabled v-model="ruleForm.includedAmount" placeholder="请输入含税金额">
                    <template slot="append">元</template>
                  </el-input>
                </el-form-item>
              </el-col>
          ...
          <el-row :gutter="24">
            <el-col :span="10">
              <el-form-item label="项目名称" prop="projectName">
                <el-input disabled v-model="ruleForm.projectName" placeholder="请输入项目名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="10" :offset="2">
              <el-form-item label="项目编号" prop="projectCode">
                <el-input disabled v-model="ruleForm.projectCode" placeholder="请输入项目编号"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            <div class="border-top">
              ...
              <el-col :span="22">
                <el-form-item label-width="50px">
                  <div v-if="!['view','detail'].includes($route.query.type)">
                    <el-button
                      type="primary"
                      size="small"
                ...
                >
                  ...
                  <el-table-column prop="remindMatter" label="提醒事项">
                    <template v-slot="{row}">
                      <el-input :disabled="['view','detail'].includes($route.query.type)" v-model="row.remindMatter"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column label="提醒时间" width="160">
                    <template v-slot="{row}">
                      <el-date-picker
                        :disabled="['view','detail'].includes($route.query.type)"
                        style="width: 100%"
                        type="date"
...
import {getContractType, getOrgContract, postEidtSing} from "@/service/approval";
import {formatSeconds, getTableData, OpenKgWindow, setTableData, setUplod} from '@/utils/tools'
import {getTemplateInfos} from "@/service/template";
import {saveFulfill,saveSnapFulfill} from "@/service/assistant";
...
export default {
  ...
  mounted() {
    const {type, statue} = this.$route.query;
    if (statue === 'STA002') {
      this.postEidtSing()
    } else {
      if (type === 'detail') {
  ...
  methods: {
    ...
    setRulform(data) {
      ...
      this.ruleForm = {
        ...
        memberName: data.ecmBusinessMemberList[0] && data.ecmBusinessMemberList[0].memberName || '',
        phone: data.ecmBusinessMemberList[0] && data.ecmBusinessMemberList[0].phone || '',
        ...
        originalCurrency: data.originalCurrency / 100 || '',
        excludedAmount: data.excludedAmount / 100 || '',
     ...
     */
    async handleSubmit() {
      const {id, statue} = this.$route.query;
      // 临时表
      if (statue === 'STA002') {
        const {code} = await saveSnapFulfill({
          contractId: id,
          contractFulfillList: this.tableData.length > 0 && setTableData(this.tableData) || [],
        })
        if (code === '000000') {
          this.$message.success('处理成功')
          this.$router.push({name: 'toolremind'})
        }
      } else {
        const {code} = await saveFulfill({
          contractId: id,
          contractFulfillList: this.tableData.length > 0 && setTableData(this.tableData) || [],
        })
        if (code === '000000') {
