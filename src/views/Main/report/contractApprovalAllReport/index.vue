<template>
    <div>
        <div id="contractApproAllChart" class="echart2" :style="drawStyle" @click="showTableLevel"></div>
              ...
              <el-table-column v-for="(item,index) in legends" :key="index" :label="item">
                  <template slot-scope="scope">
                      <el-link @click="getMoreInfo(apiData[scope.$index])">
                        {{apiData[scope.$index].indexData}} (小时)
                      </el-link>
                  </template>
              </el-table-column>
            </el-table>
...
export default {
    ...
    methods:{
        ...
        queryData(){
            ...
            this.reportData = []
            this.dataTitle = []
            reportApi.queryBySigleContractCountManager(this.queryParams).then(res=>{
                if(res.code === '000000'){
                    this.reportData = []
                    this.dataTitle = []
        ...
        drawLine(){
            ...
            let myChart = this.$echarts.init(document.getElementById('contractApproAllChart'))
