    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="form-bg">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="125px" class="demo-ruleForm">
          ...
          <el-row :gutter="24">
            <el-col :span="22">
              <el-form-item label="线下部分扫描件" prop="pass">
                <common-upload
                  v-on:get-file-list="getBatchScanningAttamentList"
                  :show-attached-list="showBatchScanningAttamentList"
                  content="1.支持扩展名：仅支持上传单个pdf文件或 .jpg .png，请确保扫描件清晰、顺序正确无缺页 大小不超过50M；"
                  title=""
                  isPdfShow
                />
              </el-form-item>
...
import { getContractType, postEidtSings, getBatchUpload } from '@/service/approval'
...
export default {
  ...
  data () {
    return {
      ...
      showAttachedList: [],
      showWorkList: [],
      batchScanningAttamentList: [],
      showBatchScanningAttamentList: [],
    }
  },
  created () {
  ...
  methods: {
    ...
    async setRuleForm (data) {
      ...
      this.showBatchScanningAttamentList = data.batchScanningAttamentList;
      this.ruleForm = {
        ...this.ruleForm,
    ...
    getBatchScanningAttamentList (list) {
      list.forEach((item => {
        return item.contractId = this.$route.query.id || ''
      }))
      this.batchScanningAttamentList = list
    },
    /**
     *  保存
     */
    async handleNextStep () {
      const { code, data } = await getBatchUpload({
        contractId: this.ruleForm.contractId,
        batchScanningAttamentList: this.batchScanningAttamentList
      })
      if (code === '000000') {
          this.$router.go(-1)
      }
