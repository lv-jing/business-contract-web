<template>
  <div class="temlateDetail">
    <div class="bg-white">
      <el-card class="box-card" shadow="never">
        <div slot="header" class="clearfix flex flex-center-between">
          <b class="ft18">范本详情</b>
          <div style="float:right;">
            <el-button @click="addToCommonUse">添加常用</el-button>
            <el-button type="primary" @click="downLoadTemplate">下载</el-button>
            <el-button
              type="primary"
              v-show="!isChooseLowVersion"
...
export default {
  ...
  methods: {
    ...
    // 废止
    async repealTemplate() {
      let params = {
        templateId: this.templateForm.templateId,
      };
      const { code, data } = await adminRepealTemplate(params);
      if (code === "000000") {
        this.$message({ type: "sueccess", message: "废止成功！" });
     ...
     * 清除未优化成功的范本
     */
    async deleteOpziTemplate(){
      const{code,data} = await deleteOpziTemplate(this.templateForm)
      if(code === '000000'){
        this.$message({ type: "sueccess", message: "清除成功！" });
      }
    },
    // 关闭
