...
import {postBatchList, postBactDetail, getBatchsList} from '@/service/approval'
...
export default {
  ...
  methods: {
    ...
    async getDraftsList() {
      ...
      const {code, data} = await getBatchsList({
        ...this.searchForm,
        batchId: id
      })
      if (code === '000000') {
    ...
    getApprovlStart(params) {
      this.postBatchList(params)
    },
    /**
     * 保存
     */
    async postBatchList(params) {
      const {id} = this.$route.query
      const {code} = await postBatchList({
        ...params,
        batchId: id
      })
      if (code === '000000') {
